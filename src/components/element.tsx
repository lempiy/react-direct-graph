...
> {
    ...
    getNodeHandlers(): { [eventName: string]: (e: React.MouseEvent) => void } {
        ...
        if (onNodeClick) handlers.onClick = this.wrapEventHandler(
            onNodeClick,
            node,
            incomes
        )
        if (onNodeMouseEnter) handlers.onMouseEnter = this.wrapEventHandler(
            onNodeMouseEnter,
            node,
            incomes
        )
        if (onNodeMouseLeave) handlers.onMouseLeave = this.wrapEventHandler(
            onNodeMouseLeave,
            node,
            incomes
        )
    ...
    getLineHandlers(
        ...
        if (onEdgeClick) handlers.onClick = this.wrapEventHandler(onEdgeClick, node, [
            income
        ])
        if (onEdgeMouseEnter) handlers.onMouseEnter = this.wrapEventHandler(
            onEdgeMouseEnter,
            node,
            [income]
        )
        if (onEdgeMouseLeave) handlers.onMouseLeave = this.wrapEventHandler(
            onEdgeMouseLeave,
            node,
            [income]
        )
