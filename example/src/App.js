...
export default class App extends Component {
    ...
    onEdgeClick = (event, node, incomes) => {
        ...
        if (node.isAnchor) {
            const nodeItems = this.extractAnchor(node)
            node = nodeItems.node
            income = nodeItems.income
        }
        if (income.isAnchor) {
            const anchorItems = this.extractAnchor(income)
            node = anchorItems.node
            income = anchorItems.income
        }
